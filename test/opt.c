...
int main() {
    const char* argv[] = {"opt", "hello", "world", "--width", "1024", "-x", "0", "--string-example", "aboba", "end"};
    const int argc  = (int) (sizeof(argv) / sizeof(argv[0]));

    uint32_t width = 0;
    bool flag_example = false;
    const char* string_example = NULL;
    struct option example_options[] = {
        { "-w", "--width", OPT_UINT32, &width },
        { "-x", "--flag-example", OPT_FLAG, &flag_example },
        { "-s", "--string-example", OPT_STRING, &string_example },
    };
    ...
    if (loaded_mask > 0) {
        ...
        Test( options_index_was_loaded(7, loaded_mask)); // "--string-example"
        Test( options_index_was_loaded(8, loaded_mask)); // "aboba"
        Test(!options_index_was_loaded(9, loaded_mask)); // "end"
        Test(width == 1024);
        Test(flag_example == true);
        Test(0 == strcmp(string_example, "aboba"));
        return num_fails;
    } else {
